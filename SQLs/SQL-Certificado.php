...
if (isset($_POST['submitf'])) {
    ...
    if ((!$find1) && ($find2)) {
        ...
        $_SESSION['sql1'] = "SELECT * FROM Certificado WHERE Estado = '$find2'";
        header("location: ../Paginas/AdministrarCertificado.php");
    }

    if (($find1) && (!$find2)) {


        $_SESSION['sql1'] = "SELECT * FROM Certificado WHERE Nombre LIKE '%$find1%'";
        header("location: ../Paginas/AdministrarCertificado.php");
    }


    if (($find1) && ($find2)) {
        $_SESSION['sql1'] = "SELECT * FROM Certificado WHERE Estado = '$find2' and Nombre LIKE '%$find1%' ";
        header("location: ../Paginas/AdministrarCertificado.php");
    }
...
if (isset($_POST['submitundo'])){
    $_SESSION['sql1'] = "SELECT * FROM Certificado";
    header("location: ../Paginas/AdministrarCertificado.php");
}
...
if (  (isset($_POST['submita'])) || (isset($_POST['submited'])) || (isset($_POST['submite'])) ) {
    ...
    if (isset($_POST['submited'])) {
        ...
        $id_Certificado = $_POST['Id_certificadoe'];
    ...
    if (isset($_POST['submite'])) {
        ...
        $id_Certificado = $_POST['Id_certificadoel'];
    ...
    if (($accion == "Eliminar") || ($accion == "Editar")) {
        ...
        $sql = "SELECT * FROM Certificado WHERE Id_certificado = '$id_Certificado'"; //Filtro con correo en la base de datos
        ...
        $sql = "SELECT * FROM Certificado WHERE Nombre = '$Nombre'"; //Filtro con correo en la base de datos
        $resultado = mysqli_query($conn, $sql);
        $rowCount = mysqli_num_rows($resultado); //Obtener las lineas obtenidas de la base de datos con el resultado obtenido despues del filtro
    ...
    if ($rowCount > 0) {
        array_push($errors, 'Ya existe un registro con la informaci&oacuten ingresada');
    }
    ...
    if (count($errors) <= 0) {
        ...
        if ($accion == 'Agregar') {
            ...
            $sql = "INSERT INTO Certificado  (Nombre,Estado) VALUES (?,?) ";
        ...
        if ($accion == 'Editar') {
            ...
            $sql = " UPDATE `Certificado` SET `Nombre` = '$Nombre',`Estado` = '$Estado' WHERE `Certificado`.`Id_certificado` = $id_Certificado";
        ...
        if ($accion == 'Eliminar') {
            ...
            $sql1 = "SELECT * FROM Curso WHERE Id_certificado= '$id_Certificado'"; //Filtro con correo en la base de datos
            ...
            if ($rowCount > 0) {
                ...
                $sql = "DELETE FROM certificado WHERE Id_certificado = $id_Certificado";
                $stmt = mysqli_stmt_init($conn);
                $prepareStmt = mysqli_stmt_prepare($stmt, $sql);
